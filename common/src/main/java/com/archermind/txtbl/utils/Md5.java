...
public class Md5 {
	...
	private static byte padding[] = { (byte) 0x80, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0,
	...
	private void decode(int output[], byte input[], int off, int len) {
		...
		int j = 0;
		for (; j < len; i++, j += 4) {
			output[i] = (input[off + j] & 0xff | ((input[off + j + 1] & 0xff) << 8) | ((input[off + j + 2] & 0xff) << 16) | ((input[off
                                + j + 3] & 0xff) << 24));
		}
	}
	...
	public static String getEncryStr(String email){
