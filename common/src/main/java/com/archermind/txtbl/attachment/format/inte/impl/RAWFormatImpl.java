 ...
 * Time: 3:03 PM
 */
public class RAWFormatImpl implements AttachmentFormat {
    ...
    public List<Attachment> format(String mailbox, String msgID, String filename, byte[] dataByte, int targetHeight, int targetWidth) {
        try {
            log.info("formatting image for RAW format/RAWFormatImpl/: [" + mailbox + "] [" + msgID + "] [" + filename + "]");
            ...
            log.error("format/RAWFormatImpl/Exception: [" + mailbox + "] [" + msgID + "] [" + filename + "]", e);
        }
        return null;
    }
}
